...
require("ggrepel")
# + geom_text(vjust=1.5, hjust=0.7,label=rownames(final2))
#autoplot(final.pr, data = final2, frame = TRUE, label=TRUE, label.vjust=1.5, label.hjust=0.7,label.label=rownames(final2)) + theme_light() + geom_text_repel(aes(label=rownames(final2)), size = 3.5)
autoplot(final.pr, data = final2, frame = TRUE) + theme_light() + geom_text_repel(aes(label=rownames(final2)), size = 3.6)
#text()
